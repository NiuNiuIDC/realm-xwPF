...
validate_rule_ids() {
    local rule_ids="$1"
    # 重置数组，避免污染
    unset valid_ids invalid_ids
    local valid_ids=()
    local invalid_ids=()

    # 1. 先把区间 1-99 展开成逗号列表
    local expanded_ids=""
    IFS=',' read -ra parts <<< "$rule_ids"
    ...
    # 2. 只保留“真实存在”的ID，不存在的不算invalid，直接扔掉！
...
batch_delete_rules() {
    ...
    if [ "$invalid_count" -gt 0 ]; then
        echo -e "${YELLOW}警告：以下ID无效/不存在，已跳过: $invalid_ids${NC}"
    fi

    # 没有有效ID才退出
    if [ "$valid_count" -eq 0 ]; then
        echo -e "${RED}错误：没有找到任何有效规则ID${NC}"
        return 1
    fi
    ...
    echo -e "${YELLOW}即将删除以下 ${valid_count} 个规则:${NC}"
    echo ""
    for id in "${valid_ids_array[@]}"; do
        local rule_file="${RULES_DIR}/rule-${id}.conf"
        if read_rule_file "$rule_file"; then
            echo -e "${BLUE}ID:${RULE_ID} | ${GREEN}$RULE_NAME${NC} | 端口: $LISTEN_PORT${NC}"
        fi
    done
    echo ""
    ...
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        ...
        echo ""
        echo -e "${GREEN}批量删除完成，共删除 $deleted 个规则${NC}"
        echo -e "${BLUE}正在重排序...${NC}"
        if reorder_rule_ids; then
            echo -e "${GREEN}✓ 排序完成${NC}"
        fi
        return 0
    else
        echo "已取消"
