parse_port_range() {
    ...
    for chunk in "${PORT_CHUNKS[@]}"; do
        ...
        if [[ "$chunk" =~ ^([0-9]+)-([0-9]+)$ ]]; then
            ...
            # 单端口格式验证
            if ! [[ "$chunk" =~ ^[0-9]+$ ]] || [ "$chunk" -lt 1 ] || [ "$chunk" -gt 65535 ]; then
                echo -e "${RED}端口 $chunk 无效（需 1-65535）${NC}"
                return 1
            fi
            parsed_ports+=("$chunk")
        fi
    ...
    # 去重（避免重复端口）
    parsed_ports=($(echo "${parsed_ports[@]}" | tr ' ' '\n' | sort -nu | tr '\n' ' '))
    # 输出解析后的端口（空格分隔）
    echo "${parsed_ports[*]}"
    return 0
}
# 为中转服务器创建多端口规则（改后：不刷屏，只显示端口段）
create_nat_rules_for_ports() {
    local listen_ports="$1"
    local remote_ports="$2"
    local origin_nat_port="$3"
    local origin_remote_port="$4"

    listen_ports=$(echo "$listen_ports" | tr -d ',' | tr -s ' ' ' ')
    remote_ports=$(echo "$remote_ports" | tr -d ',' | tr -s ' ' ' ')
    ...
    # 👉 只打印一次汇总，不刷屏
    echo -e "${BLUE}✅ 中转规则创建完成 | 入口端口段: $origin_nat_port → 出口端口段: $origin_remote_port | 共 $listen_count 条规则${NC}"
}

create_exit_rules_for_ports() {
    local listen_ports="$1"
    local forward_ports="$2"
    local origin_exit_port="$3" # 新增：原始端口段

    # 解析端口列表
    listen_ports=$(echo "$listen_ports" | tr -d ',' | tr -s ' ' ' ')
    forward_ports=$(echo "$forward_ports" | tr -s ' ' ' ')
    IFS=' ' read -ra LISTEN_PORT_ARRAY <<< "$listen_ports"
    ...
    # 👉 只打印一次汇总，不刷屏
    echo -e "${BLUE}✅ 服务端规则创建完成 | 端口段: $origin_exit_port | 共 $listen_count 条规则${NC}"
}
...
configure_nat_server() {
    ...
    echo ""
# 解析端口段/离散端口为完整端口数组
# 支持格式：8080（单端口）、8080,8081（离散）、8080-8090（段）、混合（8080,8085-8090）

# 替换原有的 NAT_LISTEN_PORT 输入逻辑
echo -e "${BLUE}支持格式：单端口(8080)、多端口(8080,8081)、端口段(8080-8090)、混合(8080,8085-8090)（回车随机单端口）${NC}"
while true; do
    read -p "请输入本地监听端口 (客户端连接的端口，nat机需使用分配的端口): " NAT_LISTEN_PORT
    # 👉 关键：保存**原始输入**（端口段），后面显示用
    ORIGIN_NAT_PORT="$NAT_LISTEN_PORT"
    # 随机端口（仅单端口）
    ...
    # 解析端口段/离散端口（后台生成端口数组，用于创建规则）
    parsed_ports=$(parse_port_range "$NAT_LISTEN_PORT")
    if [ $? -eq 0 ] && [ -n "$parsed_ports" ]; then
        NAT_LISTEN_PORT="$parsed_ports" # 解析后的端口数组（后台用）
        # 👉 只显示**原始端口段**，不显示展开端口
        echo -e "${GREEN}端口段解析完成: $ORIGIN_NAT_PORT${NC}"
        break
    else
        echo -e "${RED}无效端口格式，请重新输入${NC}"
    fi
done
# 调整多端口检测逻辑（现在是空格分隔）
local is_multi_port=false
local port_status=0
IFS=' ' read -ra PORT_ARRAY <<< "$NAT_LISTEN_PORT"
if [ ${#PORT_ARRAY[@]} -gt 1 ]; then
    ...
    # 单端口检测
    check_port_usage "${PORT_ARRAY[0]}" "中转服务器监听"
    port_status=$?
fi
    ...
    echo -e "${BLUE}支持格式：单端口(8080)、多端口(8080,8081)、端口段(8080-8090)、混合(8080,8085-8090)${NC}"
    while true; do
        read -p "出口服务器的监听端口(多端口/端口段): " REMOTE_PORT
        # 保存原始输入的端口段，用于显示
        ORIGIN_REMOTE_PORT="$REMOTE_PORT"
        ...
        # 解析端口段/离散端口（和入口端口用同一个parse_port_range函数）
        parsed_remote_ports=$(parse_port_range "$REMOTE_PORT")
        if [ $? -eq 0 ] && [ -n "$parsed_remote_ports" ]; then
            REMOTE_PORT="$parsed_remote_ports" # 解析后的端口数组，后台用
            echo -e "${GREEN}端口段解析完成: $ORIGIN_REMOTE_PORT${NC}"
            break
        else
            echo -e "${RED}无效端口格式，请重新输入${NC}"
        fi
    ...
    # 测试连通性（只对单端口测试，多端口/端口段跳过）
    local connectivity_ok=true
    # 检查是否为多端口/端口段（解析后端口数>1）
    IFS=' ' read -ra REMOTE_PORT_ARRAY <<< "$REMOTE_PORT"
    if [ ${#REMOTE_PORT_ARRAY[@]} -gt 1 ]; then
        echo -e "${BLUE}多端口/端口段配置，跳过连通性测试${NC}"
    else
        echo -e "${YELLOW}正在测试与出口服务器的连通性...${NC}"
        if check_connectivity "$REMOTE_IP" "${REMOTE_PORT_ARRAY[0]}"; then
    ...
    echo ""
    # 👉 传原始端口段，不是展开端口（同时传入口和出口的原始端口段）
    create_nat_rules_for_ports "$NAT_LISTEN_PORT" "$REMOTE_PORT" "$ORIGIN_NAT_PORT" "$ORIGIN_REMOTE_PORT"
}
...
configure_exit_server() {
    ...
    while true; do
        ...
        # 保存原始输入的端口段，用于后续汇总显示
        ORIGIN_EXIT_PORT="$EXIT_LISTEN_PORT"

        if [[ -z "$EXIT_LISTEN_PORT" ]]; then
            echo -e "${RED}端口不能为空${NC}"
            continue
        fi

        # 调用全局的 parse_port_range 解析端口段
        parsed_exit_ports=$(parse_port_range "$EXIT_LISTEN_PORT")
        if [ $? -eq 0 ] && [ -n "$parsed_exit_ports" ]; then
            EXIT_LISTEN_PORT="$parsed_exit_ports" # 解析后的端口列表，后台用
            echo -e "${GREEN}端口段解析完成: $ORIGIN_EXIT_PORT${NC}"
            break
        else
            echo -e "${RED}无效端口格式，请重新输入${NC}"
        fi
    ...
    # 跳过端口占用检测（多端口/端口段）
    IFS=' ' read -ra EXIT_PORT_ARRAY <<< "$EXIT_LISTEN_PORT"
    if [ ${#EXIT_PORT_ARRAY[@]} -gt 1 ]; then
        echo -e "${BLUE}检测到多端口/端口段配置，跳过端口占用检测${NC}"
    else
        check_port_usage "${EXIT_PORT_ARRAY[0]}" "服务端监听"
    fi
    ...
    echo -e "${BLUE}支持格式：单端口(8080)、多端口(8080,8081)、端口段(8080-8090)、混合(8080,8085-8090)${NC}"
    while true; do
        read -p "转发目标业务端口(多端口/端口段): " FORWARD_PORT
        # 保存原始输入的端口段，用于后续显示
        ORIGIN_FORWARD_PORT="$FORWARD_PORT"
        ...
        # 调用全局的 parse_port_range 解析端口段
        parsed_forward_ports=$(parse_port_range "$FORWARD_PORT")
        if [ $? -eq 0 ] && [ -n "$parsed_forward_ports" ]; then
            FORWARD_PORT="$parsed_forward_ports" # 解析后的端口列表，后台用
            echo -e "${GREEN}端口段解析完成: $ORIGIN_FORWARD_PORT${NC}"
            break
        else
            echo -e "${RED}无效端口格式，请重新输入${NC}"
        fi
